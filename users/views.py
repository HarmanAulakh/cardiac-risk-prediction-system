...
from heart_disease_prediction_system.utils import getDropDown, dictfetchall
import log
...
#             # value = request.session.get('user_username')
            
#             response = HttpResponse("hkhkgjg")
            
#             response.set_cookie('name', request.session['user_id'] )
#             return response
#     else:
...
def index(request):
    ...
    if(request.method == "POST"):
        ...
        if(getUser.user_password == request.POST['password']):
            ...
            log.loggedInUser=getUser.user_id
            request.session['user_level_id'] = 1
...
def add(request):
    ...
    "citylist":city.objects.all(),
    "rolelist":role.objects.all(),
    "topicslist":getDropDown('topics', 'topics_id', 'topics_name',0, '1'),
    "courselist":getDropDown('course','course_id', 'course_name', 0, '1'),
    "heading":'Users',
    "sub_heading": 'Users',
    "statelist":state.objects.all(),
    "countrylist":country.objects.all()
...
def changepassword(request):
    if (request.method == "POST"):
        try:
            if(request.POST["user_new_password"] == request.POST["user_confirm_password"]):
                addUser = user(
                ...
                messages.add_message(request, messages.INFO, "Password do not match. TRY AGAIN !!!")
        except Exception as e:
            return HttpResponse('Something went wrong. Error Message : '+ str(e))
